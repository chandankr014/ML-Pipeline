...
from scripts.imputer import SeasonalRegressorImputer
...
def main(CITY):
    ...
    try:
        # 1. Data Loading
        FILE = f"data/{CITY}_data_processed.csv"
        USE_COLS = ['year','month','U10','V10','WS','WD','SP','BLH','TCC','T2M','D2M','RH','SSR','TP', 'PM25_WUstl']
    ...
    try:
        ...
        numeric_transformer = Pipeline(steps=[
            ...
            ('seasonal_imputer', SeasonalRegressorImputer(time_col='month')),
    ...
    try:
        ...
        results_dir = "df_splits"
        os.makedirs(results_dir, exist_ok=True)
        X_train.to_csv(os.path.join(results_dir, f"X_train_{CITY}.csv"), index=False)
        y_train.to_csv(os.path.join(results_dir, f"y_train_{CITY}.csv"), index=False)
        X_test.to_csv(os.path.join(results_dir, f"X_test_{CITY}.csv"), index=False)
        y_test.to_csv(os.path.join(results_dir, f"y_test_{CITY}.csv"), index=False)
    ...
    try:
        ...
        # exclude_models = {"RNN", "LSTM", "BI-LSTM", "GRU", "GAM", "CERT"}
        exclude_models = {}
    ...
    results = []
    predictions_df = pd.DataFrame({"y_test": y_test.values})
    r2_train_dict = {}
    r2_test_untuned_dict = {}
    r2_test_tuned_dict = {}
    for name, model in models.items():
        ...
        try:
            ...
            predictions_df[f"{name}_untuned"] = y_pred
            r2_train = r2_score(y_train, y_pred_train)
            r2_test = r2_score(y_test, y_pred)
            r2_train_dict[name] = r2_train
            r2_test_untuned_dict[name] = r2_test
            ...
            logging.info(f"R²: {r2_test:.4f}, RMSE: {rmse:.4f}, MAE: {mae:.4f}, R² train: {r2_train:.4f}")
        except Exception as e:
            logging.error(f"{name} failed: {e}")
            continue
    pd.DataFrame(results).to_csv(f"Model_Results_UNTUNED_{CITY}.csv", index=False)
    ...
    for name, model in models.items():
        ...
        try:
            if name in tuning_results and "error" not in tuning_results[name]:
                ...
                best_cv_score = tuning_results[name].get('best_score', None)
                if best_params is not None:
                    try:
                        ...
                        y_pred = pipeline.predict(X_test)
                        model_type = "Tuned"
                        predictions_df[f"{name}_tuned"] = y_pred
                        r2_test_tuned_dict[name] = r2_score(y_test, y_pred)
                    except Exception as e:
            ...
            if pipeline is None:
                ...
                pipeline.fit(X_train, y_train)
                y_pred = pipeline.predict(X_test)
                # If tuning failed, use untuned value for after_tuning
                r2_test_tuned_dict[name] = r2_score(y_test, y_pred)
            ...
            if model_type == "Tuned" and best_cv_score is not None:
                logging.info(f"Test R²: {r2:.4f}, RMSE: {rmse:.4f}, MAE: {mae:.4f}, Best CV R² after tuning: {best_cv_score:.4f}")
            else:
                logging.info(f"Test R²: {r2:.4f}, RMSE: {rmse:.4f}, MAE: {mae:.4f}")
        except Exception as e:
            logging.error(f"{name} ({model_type}) failed: {e}")
            continue
    pd.DataFrame(final_results).to_csv(f"Model_Results_TUNED_{CITY}.csv", index=False)
    predictions_df.to_csv(f"results/all_predictions_{CITY}.csv", index=False)
    ...
    for name in models.keys():
        r2_summary.append({
            "Model": name,
            "r2_on_train": r2_train_dict.get(name, None),
            "r2_before_tuning": r2_test_untuned_dict.get(name, None),
            "r2_after_tuning": r2_test_tuned_dict.get(name, None)
        })
    pd.DataFrame(r2_summary).to_csv(f"results/model_r2_summary_{CITY}.csv", index=False)
...
if __name__ == "__main__":
    ALL_CITIES = [
        ...
        # 'indore', 'kolkata', 
        # 'ludhiana', 'surat', 'thiruvananthapuram', 'visakhapatnam'
    ]
