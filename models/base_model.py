...
class BaseModel(ABC):
    ...
    def get_optimized_param_grid(self) -> Dict[str, Any]:
        """
        Return optimized parameter grid with < 1000 combinations.
    ...
    def get_search_strategy(self) -> str:
        ...
        Returns:
            'grid' for < 1000 combinations, 'random' otherwise
        """
        param_count = self._calculate_param_combinations()
        return 'grid' if param_count < 1000 else 'random'
    ...
    def tune_hyperparameters(self, 
        ...
        if strategy == 'grid':
            print(f"Using GridSearchCV (combinations < 1000)")
            search = GridSearchCV(
                pipeline,
