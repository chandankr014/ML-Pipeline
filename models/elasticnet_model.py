...
class ElasticNetModel(BaseModel):
    ...
    def get_optimized_param_grid(self) -> Dict[str, Any]:
        """
        Return optimized parameter grid with reduced combinations for ElasticNet.
        
        Returns:
            Dictionary of optimized parameters for ElasticNet
        """
        return {
            'regressor__alpha': [0.01, 0.1, 1.0, 10.0],         # 4 values
            'regressor__l1_ratio': [0.2, 0.5, 0.8],              # 3 values
            'regressor__fit_intercept': [True],                  # 1 value
            'regressor__selection': ['cyclic', 'random']         # 2 values
        }
