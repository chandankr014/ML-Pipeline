...
class CatBoostModel(BaseModel):
    ...
    def get_param_distributions(self) -> Dict[str, Any]:
        """
        Return parameter distributions for RandomizedSearchCV (legacy - medium grid).
        
        Returns:
            Dictionary of parameter distributions for CatBoost
        """
        return {
    ...
    def get_optimized_param_grid(self) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary of optimized parameters for CatBoost
        """
        return {
            'regressor__iterations': [100, 200, 300],            # 3 values
            'regressor__depth': [3, 6, 9],                       # 3 values
            'regressor__learning_rate': [0.01, 0.1, 0.2],        # 3 values
            'regressor__l2_leaf_reg': [1, 5],                    # 2 values
            'regressor__bagging_temperature': [0, 1]             # 2 values
        }
